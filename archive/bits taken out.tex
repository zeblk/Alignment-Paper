...
The alignment problem in artificial intelligence (AI) is often posed as the problem of building AI to act in accordance with human values or principles.
...
\subsection{Genes and modularity}
...
Each entity sees a different corner of the world, developing its own unique perspective over timescales of evolution or learning, and becoming a model of this niche \citep{conant1970every, friston2013life}. At the same time, boundaries limit rigid associations, so entities experience multiple contexts. They therefore tend toward developing both modularity, where they can function in multiple contexts, and generality, like a person learning multiple languages and extracting the underlying commonalities \citep{clune2013evolutionary,melo2016modularity, simon2018near}. 

The separation of timescales is the engine of evolvability. It allows a complex system to tinker with one part without crashing the others, turning the impossible probability of evolving a complex whole all at once into a manageable series of local optimizations. A module is not just a spatial compartment; it is a temporal reservoir where a specific piece of the world's structure has been successfully captured, stabilized, and made available for novel recombination. The result is a system that can reason compositionally, building infinite new meanings from a finite alphabet of robust, time-tested parts.
...
In simulations of evolution, modular discovery, with heating-cooling cycles, facilitates generalizable, robust solutions \citep{woolley2011deleterious}. By contrast, optimizing for a particular objective leads to fragile, overly-complex solutions, like codependent genes that haven't been broken up by recombination. A diverse array of modular parts can later be called on and rearranged to solve new problems.
...
\subsection{Other views on life}

Acknowledging many different ways of thinking about `life'. Thermodynamic perspective, dynamical systems perspective, natural selection perspective etc. Richness of a computational universe even before cells and DNA. Explaining how collapse and barriers fits into this. 

Assembly theory.

More about theories of life: https://www.cambridge.org/core/journals/philosophy-of-science/article/closure-of-constraints-as-a-theoretical-model/66F5B26F146D1DF12CB1B3019B50734A

We also need to reference other accounts of what life is. Eg, \citep{bedau2023what}. And emphasize that our goal isn't to define life comprehensively, but to focus on one important aspect (barriers).

Relationship to homeostasis. Homeostatic potential is related to robustness. 
...
Even the concept of `values' is a form we might over-index on. This might sound circular and paradoxical -- that's because it is. Doesn't `should' imply values, but if so, doesn't that contradict the statement that we shouldn't overindex on values? It's a perfect example of how you get stuck with any particular forms.
...
There is no well-defined edge of what is `us' and what is `not us'.
...
open-ended discovery (without a single objective) generates more forms. Tim Rocktäschel gave the examples of how jaw bones led to the middle ear; radar led to microwave ovens; RL led to LLM RLHF. 
...
\subsubsection{The example of inverse methods}
...
% [what about the counterargument of something like lock-in to DNA among lifeforms on Earth? well, maybe it's just part of the pulsating. in some sense DNA lock-in might be as ``bad" as the lock-in we're currently seeing to western culture across the world. it is a loss, and it probably does make earth more vulnerable and less vital, but at the same time you can't have everything.]


\subsubsection{more details about safety methods...}
...
\textit{Mitigating goal lock-in.} Improving our mechanistic understanding of the agent's internal goals, reasoning, and representations, so we can detect and correct problems at a mechanistic level \citep{olah2020zoom, burns2022discovering, bereska2024mechanistic, anthropic2024mapping}.
...
\textit{Mitigating conceptual monoculture.} Ensemble and mixtures of experts might be a small step in the right direction \citep{lakshminarayanan2017simple, shazeer2017outrageously}. Pluralistic alignment as an answer to conceptual monoculture \citep{sorensen2024roadmap}.
...
\subsection{Drives, goals and motivation}
...
In this section we first recast some well-studied AI alignment problems in this language. We then use our framework to examine what is missing from the current research landscape. Finally, we investigate what a truly aligned future could look like.
...
The existing alignment methods are all potentially valuable. Research progresses by expanding our ontologies and refining our assumptions. 
...
Rather than referring to a particular concept, `human values' is more like a boundary on our own reference frame: holding it as useful while only part of the whole picture.

Just as problems of alignment can be viewed as problems of overcommitment, the methods being developed in AI safety and alignment research can be viewed as boundaries limiting overcommitment. 
...
perhaps memories or parts of representation space kept separate by design or emergently through learning. Suppose Gemini, ChatGPT, Minimax, Grok, Claude and DeepSeek diverged into totally different entities, using different architectures, different training data and so on. A person could then weigh the ensemble of their responses for improved answer. (although cf the Platonic Representation Hypothesis.)

New things have been arising in the universe for a long time. A billion years ago there were no plants. Four years ago there was no ChatGPT. 

The lens we propose in this paper, of `misalignment as overcommitment', is itself a myopic form. Alignment intrinsically cannot be fully understood. 


Another interesting example is that maybe human values themselves favor richness and avoiding collapse. Deep human values favor non-collapsed situations because that's the ultimate long-sightedness arising from meta-learning.
...
For example, the weathering of newly formed rocks made minerals available for life. In some theories, the cyclic proto-metabolic chemistry of deep sea vents was the dynamical substrate that emerging life rode on top of.

When robust plate tectonics started about a billion years ago, this change likely favored more complex life in response to the new niches and dynamic selection pressures. What we think of as life is a smooth continuation from the rich systems of the universe as they continue to unfold.

A kind of `life force' from the statistical pressure of autocatalytic cycles and combinatorial symbiogenesis.

Some technology can even be thought of as having a kind of livingness \citep{bedau2010living}, which is tied to the rest of the world.




%======================================================================
\section{Cut from `pnas\_paper medium edits.tex' when restructuring to match the outline (2026-09-07)}
%======================================================================

\subsection{General intro}

We do not argue for or against AI development or for or against specific alignment methods. Instead, we examine principles that generalize across a breadth of living systems, with the hope they might generalize even to futures substantially different from the present.

% Scratch notes from the general intro, resolved by the outline's preview box:
...
First, that this is a general principle of life: that to be healthy it must avoid getting stuck on particular arrangements, even dynamic ones.


\subsection{Section 1 intro (scratch notes, mostly on `short-circuiting', which the outline drops)}
...
\subsection{Section 1 `Summary' subsection (became `Other examples'; the rest was cut)}
...
\subsection{From performance to alignment (whole subsection; the paragraph on industry safety practices moved to `Going forward')}
...
\subsection{Failure modes not in the outline}
...
% From `Perverse instantiation'; the idea is re-expressed in the outline's own words in `Is AI special?':
\ldots and the consequences are expected to worsen as systems work more autonomously, faster than humans can follow and in ways humans cannot understand.


\subsection{Human values (redundant with sentences kept in `From alignment to living alignment')}

To be aligned in any meaningful sense, AI must be lifelike. If you fixate on any particular thing, you become misaligned.


\subsection{Why no scheme can be alignment (fragments)}
