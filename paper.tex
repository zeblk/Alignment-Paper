...
It's not easy to access what we really value. Our stated preferences, or those revealed through our choices, are typically:
\begin{itemize} 
...
We can add deliberation. 

Philosophers talk about `coherent extrapolated volition' or `idealized values' (cites). 

However, values cast in that way are not very satisfying. As we described above, when values are formalized, they are vulnerable to proxy failure \citep{john2023dead,kurth2023replay}. If we think we've written down what we think we value, and then someone does a good enough job giving us the thing we said we want, the outcome is inevitably harmful in a broader sense. So the values, formalized this way, can't be what we most deeply value.
...
Here's a key question: are we arguing that all humans actually have the same deep values??? How do we connect this to the pluralistic alignment problem?
...
\begin{itemize}
    ...
    \item \textit{Always changing.} We are discovering new values, but also even new concepts and new kinds of things to value.
