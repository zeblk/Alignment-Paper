...
\zeb{more emphasis on the link between this section and the `toward an aligned future' section. it's not going to be obvious for readers, so we have to say it clearly.}
...
\zeb{should we condense these five examples? do we need so much detail? we may also need to adjust the language so it's more clear how we're using them to highlight the pattern of misalignment being collapse.}
...
\zeb{in a sense the central question of the paper is this: is AI excessively giving us what we want? is it excessively increasing correlation on the planet? OR, are new barriers and diversity are springing up just as fast as capabilities? even though some things are more correlated, there's also whole new dimensions for things to live in where there's a lot of diversity? it might be analogous to DNA dominating the world, arguably making things very correlated at that level, but then the diversity grew in other niches. is there space on earth for worlds to grow inwards? which version do we live in? and, do we have a choice about it? this might be the most important thing to be thinking about right now.}

\zeb{as others have pointed out, human needs (as we conceive of them now) and human-style intelligence probably aren't the ultimate thing in the universe forever. **at the same time**, human intelligence probably does have more depth and subtlety compared to trilobites (without falling into scala naturae). diverse, dynamic societies probably are probably flourishing more than a post-nuclear holocaust with people scrounging for food. }
