...
\section{Boundaries in the natural and human world}
...
In the previous section, the traveling salesman problem was selected because it has a diverse space of possible solutions and sub-solutions, so the existence of multiple approaches is valuable. Nevertheless, it pales in comparison to the diversity in the space of solutions that exists in the real world. For billions of years, genomes have been faced with an incomprehensibly vast number of kinds of problems, and explored combinatorially many partial solutions. These problems exist within a cell, within an organism, across a population.
...
\subsection{Misaligned means overcommitted}
...
\subsubsection{Elephants and giraffes}
...
What does the opposite of overcommitment look like in a future shaped by AI? In living systems, semi-permeable boundaries continunally contextualize partial forms to be more long-sighted and support increasing subtlety. Evolving boundaries supporting the expansion of nuance and light, playful interactions

there is a unique risk for overcommitment because of the extremeties attached to AI. Given this problem, how could we actually have an aligned future?
...
\subsection{Alignment to human values?}

Before we go on to describe what an aligned future could look like, let's take a short detour to address a question that may be on some readers' minds. If this level of detail is not interesting, feel free to skip ahead. The question is: when we talk about alignment, are we talking about alignment of AI to human values? 

Iris Murdoch says morality is continuously discovered \citep{murdoch2013sovereignty}.

People arguing for an expanding version of human values: \citep{yudkowsky2004coherent, singer1981expanding,russell2019human}. These are not necessarily saying that it will keep expanding endlessly, but that what we should target is some kind of as-yet-unobtained morality that would result from reflection and growth in the future. 

Consider two different definitions of `human values'. In the first definition, what we mean by `human values' is the particular values that the humans of today could reasonably express. These values are formalizable or close to formalizable. In the second definition, `human values' refers to something asymptotic and non-formalizable, more like the open-ended progression of our values as we ourselves continue to develop and evolve \citep{dewey1939theory,gadamer1960wahrheit}.
...
whatever concepts we have about it are incomplete \citep{plato2002apology, aristotle2019nicomachean, wittgenstein1922tractatus, heidegger1998humanism}.
