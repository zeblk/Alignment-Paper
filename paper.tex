...
Each distinct form -- an equation, a gene, a concept, an institution -- is myopic in the sense that it is only a small aspect of the world. Yet the existence of many different myopic forms, interacting with each other, constitutes the rich livingness of the world. Differentiation between forms, at many scales, is protected and promoted by boundaries. Without sufficient boundaries, forms overwrite each other, causing collapse into an impoverished space. Cancer spreads, concentrated power reduces human welfare, invasive species choke out complex ecosystems. 

AI couples particular myopic forms with unprecedented leverage, setting up the possibility of severe collapse. Humanity is presently pouring an unprecedented volume of energy, information and computation into AI training and inference. And excess optimization for any formal goal leads to undesired consequences. If the training objective, or the form of the model at inference time, remains fixed while the system gains increasing resources and purview, then collapse is likely.

Some researchers believe that AI is inevitably destructive for these reasons. We cautiously argue that there is a non-destructive path available, by reframing AI as a continuation of living processes that bound one another to support ongoing deepening of subtlety and structure. 

In the following sections, we use examples from the natural and human world to illustrate how boundaries place intelligent constraints on interactions and limit overwriting. Crucially, boundaries must be semi-permeable. By maintaining distinctiveness of separate forms while also allowing them to interact modularly, semi-permeable boundaries contextualize myopic forms as part of larger systems. Limiting collapse creates a delicate, ever-changing balance where many possibilities exist and new phenomena spontaneously occur. In this light, we recast alignment as an evolving network of semi-permeable boundaries that contextualize any current form of AI to support subtle functioning and ongoing arising of new, unknown structure. 
...
\section{Problem solving in groups}

In 1968, the nuclear submarine USS Scorpion vanished en route from the Mediterranean to Virginia \citep{sontag1998blind, craven2002silent, surowiecki2005wisdom}. The Navy started a search, but the amount of ocean where the vessel could be was enormous. John Craven, Chief Scientist of the U.S. Navy's Special Projects Office, devised an unusual search strategy. He assembled a diverse group of mathematicians, submarine specialists, and salvage operators. But he didn't let them communicate with each other. Each expert had to use their own methods to come up with their own estimate of where the Scorpion should be. Craven then aggregated the independent estimates into a single prediction -- and the wreckage was found only 220 yards from this spot. 

When solving problems, different people bring different perspectives and approaches. Each method processes the available data through a different set of heuristics. Under favorable conditions, combining them yields a ``wisdom of crowds" effect, which has been documented in numerous domains of problem solving \citep{surowiecki2005wisdom, condorcet1785essai}.

However, the wisdom of crowds is diminished if the group lacks diversity, either ab initio or as a result of within-group communication and influence \citep{surowiecki2005wisdom, hogarth1978note, ladha1992condorcet, hong2004groups}. Controlled experiments, as well as analyses of key decision moments in real groups, find that groups often collectively reach irrational or suboptimal solutions when diverse and dissenting viewpoints are lost to a single dominating perspective \citep{anderson1997information, stasser1985pooling, flowers1977laboratory, frey2021social, becker2017network, janis1972victims, bernstein2018intermittent}. Unstructured communication methods like open discussion have a special vulnerability of rhetorical force dominating over epistemic merit. 

At the same time, sharing information is essential for the benefits of group wisdom and cooperative behavior. The crux is transmitting information wisely: the right information at the right time, sent and received in the right way. Like selective ion channels, thoughtful strategies for communication are semi-permeable boundaries. These boundaries protect diversity while also allowing productive interaction. 

There are multiple types of effective semi-permeable boundary in groups, including: creating decentralized topologies where group members only communicate with nearby neighbors \citep{becker2017network}; defining rules that incentivize acting according to one's own belief rather than following the crowd \citep{hung2001information, bazazi2019self}; modeling the strengths and weaknesses of each group member \citep{welinder2010multidimensional}; and promoting leadership styles where one person's views are less likely to dominate \citep{flowers1977laboratory, leana1985partial}. In a later section, we will look at boundaries within an individual, such as skepticism, that make it easier to receive information without overwriting one's own beliefs.

A boundary that's especially easy to implement is simply giving members the opportunity to work independently before communicating \citep{frey2021social, surowiecki2005wisdom}. In the case of Craven's search, experts weren't allowed to communicate while forming their own estimates; the estimates were later aggregated in a principled way by Craven. In the popular and effective charrette method, a large group divides into small teams which work in isolation before aggregating partial solutions \citep{lennertz2014charrette}. Famously, Newton and Leibnitz had little exposure to the other's work while they each developed their own versions of calculus \citep{hall2002philosophers}, and they developed two sets of distinct ideas and tools which each had great value. In the words of contemporary mathematician Jean Écalle, ``I could also observe, time and again, how too deep an immersion in the math literature tended to stifle creativity" \citep{wood2023alien}.

The same effect shows up in controlled experiments. \cite{bernstein2018intermittent} tasked small groups with solving instances of the traveling salesman problem. Each group was randomly assigned to one of three conditions. In some groups, members could continually see the work of other members as they progressed toward a solution; in some groups members could only occasionally exchange progress; and in some groups there was no exchange. The researchers found that groups with continual information exchange rarely found good solutions. In these groups, typically one individual would stumble on a solution that looked compelling but was actually a dead-end (the nature of the traveling salesman problem makes these situations common). When this solution was immediately shared with others, it hampered their progress. Groups with occasional or no contact were much more likely to find optimal or near-optimal solutions. 
...
We can talk about modularity again here, too. 
...
Breaking up co-adaptation.


By making things modular, boundaries encourage each entity to have interactions with many other entities. When a system is under pressure, modularity makes each entity become more `agentic': it has to develop a wisdom that's both independent and inter-functional.
...
\section{Ecosystems: an elk's collapse is a wolf's flourishing}
...
More ideas here: https://podcasts.apple.com/gb/podcast/open-ended-ai-the-key-to-superhuman-intelligence-prof/id1510472996?i=1000671830914
...
\subsection{Beyond human-level AI}
...
% John Bowlby's attachment theory~\cite{bowlby1969, mcleod2017, cherry2023}. Bowlby proposed that infants have an innate need to form a strong emotional bond with a primary caregiver. When a caregiver is consistently responsive and available, they serve as a "secure base" from which the child can explore the world~\cite{bowlby1969, mcleod2017}. This secure base functions as the foundational external boundary. The reliability of the caregiver's presence provides the safety and security necessary for the child to venture out, experience manageable challenges, and paradoxically, develop the internal boundaries required for independence and a stable sense of self. Conversely, when caregiving is inconsistent, neglectful, or abusive, it can lead to insecure attachment styles that directly impair the development of healthy boundaries~\cite{cherry2023, attachmentproject2023, clarity2023}.

% Development of self-identity: Susan Harter ~\citep{harter1999construction}. In early childhood, the self-concept is concrete and fragmented, based on observable characteristics and specific behaviors such as, "I can run fast" or "I have brown hair"~\cite{vanDrunen2021, harter2015}. The young child's cognitive limitations prevent the formation of an integrated, overarching sense of self. As cognitive abilities mature in middle childhood, children begin to use social comparison and can integrate specific behaviors into more generalized trait labels like "I am smart" or "I am good at sports"~\cite{harter2015, harter1999}. This marks the formation of more sophisticated internal organization. Finally, in adolescence, the capacity for abstract thought allows for the construction of a "formal theory of the self," where disparate traits and social roles are integrated into higher-order abstractions like "I am an intelligent person" or "I am a loyal friend"~\cite{harter2012, vanDrunen2021}.
