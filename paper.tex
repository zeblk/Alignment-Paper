...
Each form -- an equation, a genome, a viewpoint, an institution -- is myopic in the sense that it is only a small aspect of the world. But the existence of many partial forms, maintaining separate identities while flexibly working together at many scales, constitutes the rich livingness of the world. 

Collapse is a failure mode where instead of having a delicate dynamic balance between different partialities, something is taken too seriously. 

(Collapse is a failure mode where a loss of functional diversity results from forms excessively influencing one another.)

a kind of general relativity? lol. things are defined by their interactions... (`ontic structural realism')
Another way of describing collapse is that interactions become overly rigid: there is a loss of separate flexibly interacting entities. 
but the genetic overfitting/brittleness is a different picture from the idea of one thing dominating. 

** what about things getting stuck in one pattern, losing that constructive potential? getting overly stuck together is one example of this. if things are rigidly stuck together, they lose that flexibility of being many different things with partial (lightly-held) interactions. getting stuck in one overly rigid mode. 

each form or arrangement is lightly held. 

in asex, we're overly committed to that particular form (of the arrangement within a lineage). is there a `force' that binds genes together like this? 

so then different genomes (different arrangements) are different partial forms. in asex, things get overcommitted to one particular arrangement. but what drives that? it's not interactions between the genes that drive it. so it's not boundaries between genes that protect against it. 

maybe relationships are forms too. getting too stuck in one pattern of relationship is getting stuck in a particular form. 


Boundaries protect against collapse. Without sufficient boundaries, cancer spreads, concentrated power reduces human welfare, invasive species choke out complex ecosystems. 

Semi-permeable boundaries \textit{juxtapose incompatible things against each other}. As long as we \textit{keep finding the paradox}, we keep being forced to pop out a level. (This is a possible replacement for the idea that boundaries `automatically' keep things complexifying because of some internal pressure.)

AI charges particular myopic forms with enormous leverage. Humanity is presently diverting an unprecedented volume of energy, information and computation into AI training and inference. If the training objective or the form of the model at inference time remains fixed while the system gains increasing resources and purview, there is a possibility of severe collapse. Some researchers believe AI is inevitably destructive for these reasons. We cautiously argue that there is a non-destructive path available, by reframing AI as a continuation of living processes that bound one another to support ongoing deepening of the world's organization. 

In the following sections, we use examples from natural and human systems to illustrate how boundaries place intelligent constraints that limit collapse. Crucially, boundaries must be semi-permeable. By maintaining distinctiveness of separate forms while also allowing them to interact modularly, semi-permeable boundaries contextualize myopic forms as part of larger systems. In the resulting delicate dynamic balance, there are many possibilities for new phenomena. In this light, we recast alignment as an evolving network of semi-permeable boundaries that contextualize any particular form of AI to support subtle functioning and ongoing arising of new, unknown structure. 
...
Asexual reproduction follows the path of least resistance. It's available immediately, because an organism can directly copy itself without relying on anything it doesn't already have. But in asexually reproducing species, all descendants of any given organism are clones of each other. 

Being permanently locked together gives the genes excess influence on each other. Selection can't act on one gene without dragging on the others. For example, if two beneficial mutations arise in two different organisms within a lineage, they are in competition with each other. The only way for a single organism to obtain both beneficial mutations is if one arises again within the subpopulation that already carries the other, which is unlikely and therefore slow \citep{hill1966effect, felsenstein1974evolutionary}. Conversely, if a deleterious mutation arises, all of the other genes in that lineage are stuck with it forever\footnote{Unless there is a reverse mutation, which is rare.} \citep{keightley2006interference, muller1932some}. Additionally, tight linkage between genes puts direct downward pressure on genetic diversity. Suppose there are two organisms within an asexual population, carrying different alleles at two different loci. One of the loci is currently fitness-neutral while the other is subject to selection pressure. The selection pressure tends to cause one of these organisms to outcompete the other, leading to loss of diversity at the neutral locus \citep{charlesworth1993effect, hudson1995deleterious}. 

** But is this collapse into a simpler thing??? ** 
I guess it's simpler in terms of the interactions between genes being more prescribed. Instead of a 

Sexual reproduction is a boundary on excess interaction between genes\footnote{Similar logic applies to horizontal gene transfer.}. It frequently breaks up the relationships between genes, assembling them into new genomes. Even though sex is substantially costly -- an organism must find a mate in the vast and dangerous world, and half of all organisms can't reproduce \citep{smith1971origin, lehtonen2012many, smith1978evolution, speijer2015sex, goodenough2014origins} -- nearly all eukaryotes reproduce sexually. 

Boundaries support loose, flexible interactions. Like the cell membrane allows the cell to interact with different external conditions. 
...
Sex is a boundary against a particular genome dominating a lineage. It says, ``don't get overconfident in that genetic arrangement; hold each arrangement more lightly". 


Why is it beneficial to break up the monopoly of a single genome across a lineage? The most obvious reason is that when genes are recombined into new genomes, some of those genomes may have higher fitness \citep{weismann1889essays, fisher1930genetial, muller1932some, crow1965evolution}. 

There's a deep message here. By not overcommitting to a particular arrangement, sex encourages subparts (genes) to flexibly interact with many other subparts. This is a key aspect of livingness. Livingness comes from not being overly attached to a particular form or arrangement. There are many partial answers floating around, 

It's better to have a diversity of partial answers floating around, that can recombine with each other.

[wait. is a genome a partial answer, or is a gene a partial answer?]


Semi-permeable boundaries help entities develop and maintain their own separate identities. But this only matters if they are able to also interact with other entities. The crux of having a separate identity is that you are not eternally bound to one particular interaction. Modular, compositional interactions create potential for the generation of lots of new things.



Promoting modularity by having to function in different situations. ``The last section explored semi-permeable boundaries that support group members in developing and leveraging their own unique viewpoints, which in turn contribute to group-level problem solving. Sex does the same for genes.''
...
Picture a hypothetical species which reproduces asexually and has no horizontal gene transfer or other mechanisms to exchange genes between lineages. (To our knowledge, there are no such species.) The genes all being stuck together 

people working separatly on parts of a problem are modular. boundaries (they don't work together prematurely) prevent them from collapsing their diversity. 

genes are modular. it's a different kind of boundary. they're forced not to work too closely (across generations) with one particular set of other genes. 

The ``problem" is collapse: forms excessively influencing one another, causing loss of diversity. Multiple genes, with start and stop codons to transcribe a single protein, work together in a genome. But they heavily influence one another. Overfitting to each other. Muller's ratchet.

In asexually reproducing species, this is a single assembly of genes being rigidly and permanently stuck together. Boundaries contextualize that formalism, still allowing it to exist to a degree, but it doesn't have the final say anymore. Sex still allows assemblies, but now they're not absolute/permanent. By having a set of distinct (not too correlated) units, they can work together dynamically in rich ways.

In asexual reproduction, all the genes are stuck together rigidly, and that assembly is doomed to its hard-coded form for the whole lineage forever. It's like if you were forced to only think one thought forever. It's a collapse out of dynamism, pinning the genome of the lineage down to one structural form. Sexual reproduction is a boundary that contextualizes those formalisms. The formalism of the genome can still exist temporarily (eg the lifetime of one organism), and aspect of it that work well are propagated. But if it doesn't work, it's discarded and another one is tested -- like how thoughts are always evolving and being tested against each other in your brain.
...
\begin{center}
\textit{`Unity without uniformity and diversity without fragmentation.'}\newline--Kofi Annan
\end{center}
