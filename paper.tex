...
While people disagree on exactly what behaviors or properties of an AI system are aligned, there is general agreement that alignment means working toward futures that are healthy and flourishing. In the previous section we made a case that health and flourishing, in a very broad sense, are the direct opposite of overcommitment. Following this line of reasoning, we propose that alignment can be viewed as the problem of avoiding overcommitment to any particular form. No matter how good or complete our current concepts or specifications or lenses are, setting them in stone as absolutes is not aligned. A system is misaligned if it has some fundamental cap on its sensitivity to a larger context.

To give a flavor of this mapping, we start with a few examples of alignment failures that are obviously problems of overcommitment: optimizing for particular goals, concentration of human power and conceptual monoculture. Then we'll look at less obvious examples, to test the limits of the idea. We'll ask about the relationship between this broad concept of alignment and moral and normative notions of `good' and `should'. And finally we'll reflect on how large our definition makes the problem of alignment.

\subsection{Optimizing for particular goals}
...
\subsection{What about misalignment that's not overcommitment?}
