...
The alignment problem is often framed as the problem of defining what we as humans value (sometimes called `outer alignment') and then designing an AI that successfully optimizes for this function (`inner alignment'). But it is difficult to capture what we value.

The most naive approach to accessing human values is to simply ask people what they value or prefer. This approach runs into many problems \citep{amodei2016concrete,gabriel2020artificial,russell2019human}. What we say we want or like, or the choices we make, are poor reflections of what is actually advantageous for our long-term wellbeing. Our momentary preferences tend to be myopic. We often prefer to get an immediate reward even if it comes with a larger delayed punishment -- reflected in our decisions about procrastination, drug abuse, spending and so on. We lack the knowledge or resources to reason about many of the consequences of our stated preferences. Our myopia is illustrated by recent problems with sycophancy in commercial chatbots \citep{openai2025sycophancy}. Human feedback about particular bot utterances was part of the training signal for the model. But these human feedback signals tend to prefer utterances that are more flattering. It's much easier for a rater to judge surface characteristics than to determine whether the bot said something true about a complex topic, or something that would lead to increased long-run welfare. We also tend to be selfish, neglecting the welfare of other humans or living creatures. We have difficulty accessing through words or button presses all of the depth of what we value. Our stated values are extremely sensitive to the framing of the question.

To mitigate this, the field has moved toward deliberated preferences or idealized values, representing what we would prefer if we had more time, knowledge, and computational power \citep{bostrom2014superintelligence, soares2014aligning, yudkowsky2004coherent}. One way to access longer-sighted preferences is by giving humans more time and resources to think about their answer, to ask on behalf of another person, or on behalf of their future self. You can give them access to tools and information. You can ask people retrospectively whether an outcome was good, rather than prospectively. 



Safely research is also keenly attuned to the problem that different people want different things, and these things are often fundamentally irreconcilable. Pluralistic alignment. There is not even universal agreement on which principles are most appropriate for aggregating the preferences of different people.

Our values are always changing.
...
Philosophers have long emphasized the existence of tacit, embodied, and practice-based forms of understanding that resist formalization \citep{Polanyi1966, Dreyfus1972}. Ethical perception often involves sensitivity to particular contexts rather than application of general rules \citep{Nussbaum2001}. From this perspective, the problem is not merely that we have failed to find the right formalization, but that treating values as fully formalizable objects is itself a category error.
...
We began with simple, verbally stated preferences, which are obviously myopic and prone to error.

We moved toward deliberated preferences, where values are what humans would want after careful reflection. 

We expanded further to pluralistic alignment, acknowledging that no single value system can capture the diversity of human experience.

This is sometimes called the Midas problem. Whatever we specify is wrong \citep{wiener1960some}.
