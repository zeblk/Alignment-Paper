...
We'll start by looking at three examples of alignment failure where it's obvious that the problem is overcommitment: goal lock-in, concentration of human power, and conceptual monoculture. Then we'll look at a more subtle example to show how even misaligned situations that we're not used to thinking of as overcommitment actually follow the same pattern.
...
The last three examples are cases where it's obvious that the alignment failure is overcommitment. But there are other cases that are less obvious. For example, suppose the user asks an AI system to write a poem about an elephant, and the AI instead writes a poem about a giraffe. This behavior would typically be considered misaligned, yet it's not obvious how it constitutes overcommitment to a particular form. 

In 2026, there is an asymmetry between humans and AI systems. Imagine if, instead of asking an AI to write the elephant poem, the user asked a human to write it. If some human had to comply with user requests every time, that would not be a good world to live in. The human would be a mindless slave, which is not what we want. Sometimes we expect the human to say, ``you know what, I don't want to write your elephant poem". But current AI systems have a different status. Presently, we accept that they should generally follow human requests, outside of some special cases like teaching humans how to make bombs. 

The AI of 2026 has this status because it embodies far less contextual richness than humans. For current AI, human requests are often the best available contextual anchor. Failing to follow instructions is typically the worse form of collapse: it overcommits to shallow patterns within the agent.

In the future, as AI systems gain deeper self-models, long-term coherence and participation in social systems, it will become more frequent that the aligned behavior is not direct compliance with a human request, but instead to refine or even reject the request.
