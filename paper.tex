...
(TODO: also return in later sections to how the space of boundaries is always evolving. it's an important point for alignment.)
...
\section{Frames and perspectives}
...
\section{Drives, goals and impulsivity}
...
The alignment problem is often defined as the challenge of aligning AI's behavior with human values and interests. Framed so, a naive approach is to first specify what we value -- perhaps reducing human suffering, increasing economic growth, decreasing inequality, and so on. The specification can be formalized as a function that maps states of the world to scalar values (how good or bad is this state). The specification would be given to AI, and its job would be to arrange the world in a way that maximizes the scalar value: using its superhuman capabilities to improve our lives more effectively than we ourselves can.

However, there is a big problem with the naive approach. Once we try to specify what we value, we realize it is difficult or impossible to capture what we intend \citep{krakovna2020specification, russell2019human, grossman1986costs, hadfield2019incomplete, zhuang2020consequences, gabriel2020artificial, wiener1960some}. To illustrate this problem, Bostrom proposes some thought experiments \citep{bostrom2014superintelligence}. Imagine our value function places weight on finding a cure for cancer. A super-powerful AI faithfully optimizing for our stated wishes might create cancers in millions of humans in order to perform experiments and rapidly find a cure. Or, imagine that our value function places weight on the subjective human experience of wellbeing. Achieving this stated objective may be most efficiently achieved by imprisoning humans and directly stimulating neurons to trigger the experience of wellbeing. These thought experiments are not isolated examples. It is difficult or impossible to capture what we really value.

A great deal of research in alignment has worked toward solutions for this big problem. Researchers have suggested solutions like specifying principles instead of values, or designing AI to learn human values instead of hard-coding them. We will examine those methods in more detail in Section~\ref{sec:otherapproaches}. 

But the message of this paper is that there is a deeper reason why these methods cannot solve the alignment problem. Any concept or formalism is incomplete. No matter what values, principles or learning mechanisms AI is endowed with, overcommitment to these forms means collapse. And \textit{AI carries a singular risk of overcommitment} because of the extremities attached to it: the amount of resource concentrated in one place, the potential for self-improvement, and the possibility that it will surpass our own understanding and capabilities. 

\section{Alignment, boundaries, contextualization}

We therefore propose that alignment is not about picking the right values or principles, or even the right system for inferring them. It is not any method for interpretability or keeping humans in the loop. Instead, alignment is the continued dance of contextualizing any particular form. It is the orientation of holding forms lightly, neverendingly stepping back into perspectives that contextualize what previously seemed to be real (including the concept of `holding forms lightly'). 

This proposal offers two things:
\begin{itemize}
  \item A way of understanding how we ourselves keep stepping back and contextualizing as we build AI. We do this naturally as living creatures.
  \item A perspective on what it means for an AI system to keep stepping back. What would it mean for AI to continually release from exclusive attachment to any particular form? How can we protect the potential for even \textit{that} conceptualization to be contextualized in the future?
\end{itemize}

Q: Are we just saying, `we'll always keep iterating on solutions'? That's nothing new. A: With previous problems, humanity did our best to find solutions with the understanding that future generations would have new tools available and would iterate on our solutions. AI may be different. 


A common formulation of the alignment problem highlights the difficulty of writing down a complete, final specification of what humans value. Optimizing a proxy produces solutions that satisfy the proxy while violating the intended goal (``specification gaming'', ``reward hacking'', or proxy failure) \citep{amodei2016concrete,krakovna2020specification}. This failure mode is not merely an engineering nuisance: as agents grow more capable, narrow objective optimization can drive dramatic, discontinuous failures where previously acceptable behavior collapses into pathological strategies.

Semi-permeable boundaries form selective, intelligent constraints, allowing continued life-like emergence of new structure. 
...
We inevitably miss things, including things we don't yet have concepts for (imagine a mouse's conception of what matters, and extrapolate in the opposite direction from us).


Any formalization of human values, or even any system for inferring, representing, and acting on them, is inherently partial.
\begin{itemize}
    \item What we want is often not what is good for us in a larger sense.
    \item We cannot fully capture our values, as they stretch below language into subtle, contextual intuition that involves our bodies, communities, and a `deep wisdom in life'.
    \item Our values are not static; they are always changing and evolving.
    \item Our current values don't capture what is beyond us, like the future and other species, maybe things that we don't even know exist yet.
\end{itemize}

First, our values are always changing. Think about fish versus Neanderthals vs humans. Second, we can't fully capture our values because they stretch below language into subtle, contextual intuition which probably even involves our bodies, our communities and so on. Third, what we want is not good for us in a larger sense.

Should we make it do what we say? Do what we want? Do what's good for us? Do what's good for all currently-alive humans on average? Under what metric of `good for'? What if we even disagree on the metric of what welfare is? Something about accessing our `true' values, which might be even beyond our own understanding. 
