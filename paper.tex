...
However, it is worth noting that none of the studies cited here made a best effort attempt to use AI systems in a thoughtful way to increase rather than decrease diversity. This also gives hope, which we again return to in Section 2.4.
...
In a sense, this is obvious. We can't expect the thing we build today to be absolutely perfect, and we expect to keep finding and fixing problems as we recognize them. Historically, that's what we've always done: build imperfect technologies and iterate on them. But it is possible that the iterating process will not continue to work as well as it has in the past. AI already has some remarkable properties, such as rapid global adoption, intensive use of resources, concentration of information flow, anthropomorphization, offloading much cognitive activity. More speculatively, in the future AI may exhibit superhuman intelligence and recursive self-improvement, without being limited to a restrictive biological substrate. These distinctive properties may create more difficulty in iterating on imperfect solutions, compared to past technologies \citep{bostrom2014superintelligence}. There is some risk that any particular forms or assumptions built in to AI systems could gain excessive traction and be difficult to undo. This point has been extensively made for certain categories of overcommitment, particularly values. Our new observation is that there is no single solution to the problem because the nature of healthy living systems is to invent qualitatively new properties over time, and no fixed form can be aligned with this process. In other words, there's a risk if AI grows too much in power while being fixed on a particular scheme. 

Of course, this does not mean we should have no scheme! Quite the opposite. Throwing away schemes capriciously can be as much a loss of subtlety as any other particular form. Many of the existing approaches in AI safety are highly valuable, a point to which we will return in Section 2.4.
