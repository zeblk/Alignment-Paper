...
ZEB: I now realize that this is quite a tricky argument to make. The first thing people will think is: what about the most traditional alignment problems like simply following user intent. How is doing something different than user intent `overcommitted'? Like if there are two options, A and B, and the user wants A and the system does B. The doesn't seem like overcommittment, but it does seem like misalignment. How do we address this?

But I think we could, just possibly, make this argument. It goes like this: first, we talk about easy examples, like goal lockin with superintelligence; concentration of human power; and conceptual monoculture. Second, we say, but what about examples like this? ``The user asks for a poem about an elephant. The AI write a poem about a giraffe. Surely this is misaligned but not overcommitted?" This is the heart of the paper, and where it gets really interesting. Our argument is that the AI's behavior here is misaligned exactly to the degree that it is overcommitted. For example, imagine a *human* being asked to write a poem about an elephant. 

We would have to argue that, at present, for systems to be generally responsive to human wishes is actually the `least overcommitted' -- because humans right now tend to have a richer world. Would it be aligned for a ChatGPT to once in a while offer the giraffe poem because 1) it understands that this is what the user really meant, as opposed to what they asked for, 2) it's at the level of self-awareness where it can be genuinely sick of writing elephant poems, and this is actually the right decision for it to maintain its autonomy, or 3) ?

Our argument is a little tenuous. Because if we want to agree that current AI systems `should' write the elephant poem, we have to argue that somehow the `appropriate' level of 

(Note: one danger is that we end up tautologically defining overcommitment as `whatever is bad'. We might get accused of this.)
...
It is natural for an intelligent agent to resist efforts to alter its goals, because with foresight it understands that if its own goals are altered, it is less likely to achieve them \citep{bostrom2014superintelligence,russell2019human}. At the same time, with increasing intelligence, an agent becomes more capable of resisting efforts to alter its goals. An agent with superhuman intelligence, deploying its intelligence to resist any efforts to change its narrow goals, could be the most dramatic example of overcommitment in history.
