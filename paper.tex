...
In this paper, we study life to better understand how to design and relate to AI systems in ways leading to futures full of flourishing and health. In the future, humanity and AI may have changed substantially. Those future worlds may include concepts and categories that do not exist yet, perhaps even some that are incomprehensible to us at present. We therefore gravitate to ideas about alignment that could generalize over such changes. We think that studying how living systems work at many scales is one way of approaching this kind of generality.
...
\zeb{more explicit definitions. collapse, overcommitment, and short-circuiting. versus contextualization, spontaneity, potential, livingness.}

\zeb{more explicit logic: health=livingness, alignment=health, ergo livingness=alignment.}

\zeb{more explicitly make the connection between barriers and overcommitment. linking steve's point about barriers and modularity, to my point about any particular thing isn't a final answer?}
...
\zeb{any short-sighted entity doesn't place barriers in its own path. the rich diversity/separation at every scale is necessary because different \textit{things} can exist as barriers for each possible short-sighted perspective. this is everything from species competing with each other, to tectonic plates forming continents.}

\zeb{say something about different kinds of barriers and the degree of intentionality vs non-intentionality?}

\zeb{ecological barriers may have scaffolded the emergence of multicellularity, for example \citep{black2020ecological} (does that paper have any clean examples we could use?)}
...
\zeb{What can societies do that individuals can't? Why is multiagent useful?}
...
\zeb{how can we make this point? In recombination, genetic sequences are not cut arbitrarily but instead preserve genes and structural motifs. Giving the functional components a more stable identity lends itself to more elaborate organization. Making each gene a discrete `thing'.}
...
\zeb{talk about new goals? einstein quote from steve.}
...
\zeb{this section to be abridged. possibly merge with problem solving or frames and perspectives.}
