...
Now we shift gears. Across a number of living systems, we've outlined how semi-permeable boundaries place selective limits on interactions so that, instead of overcommitting to a particular form, systems maintain a delicate balance between distinctive parts that work flexibly together. With these intelligent protections, new structure continues to contextualize previous partialities. In the remaining sections, apply this lens to the AI alignment problem. 

Although there is no universally agreed definition of the alignment problem, most definitions orient on designing AI to behave in a way that is `good' rather than `bad'. Framed so, an obvious approach is to first specify a value function -- a formal definition of what is good and bad -- and then put AI to work optimizing that function. A value function might place weight on reducing human suffering, increasing wealth, decreasing inequality, and so on. 

However, as soon as we try to specify what is good, it becomes clear that it is difficult or impossible to capture what we intend \citep{krakovna2020specification, russell2019human, grossman1986costs, hadfield2019incomplete, zhuang2020consequences, gabriel2020artificial, wiener1960some}. To illustrate this problem, Bostrom proposes some thought experiments \citep{bostrom2014superintelligence}. Imagine our value function places weight on finding a cure for cancer. A super-powerful AI faithfully trying to optimize for our stated wishes could create cancers in millions of humans in order to perform experiments and rapidly find a cure. Or, imagine that our value function places weight on the subjective human experience of wellbeing. Achieving this stated objective might be most efficiently achieved by imprisoning humans and directly stimulating neurons to trigger the experience of wellbeing. 

These thought experiments are not isolated examples. It is difficult or impossible to write down a suitable value function because any concept, model or formalism is incomplete. When we try to specify values, we inevitably miss things, including things we don't yet have concepts for (imagine a mouse's conception of what matters, and extrapolate in the opposite direction from us). This was the core theme of Sections 1-\ref{sec:motivation}. If AI does too good a job of organizing the world around a particular framing of values, then the richness of the world is collapsed to that framing. 

Researchers have suggested several ways around this problem, such as specifying principles instead of values, or designing the AI to learn human values instead of having them hard-coded. We will examine these methods in more detail in Section~\ref{sec:otherapproaches}, but ultimately they run into the same problem that any overcommitting to any particular formalism or conceptualization means collapse.

A singular risk of overcommitment exists with AI because of the extremities attached to it: the amount of resource concentrated in one place, the potential for self-improvement, and the possibility that AI will surpass our own understanding and capabilities. With previous problems, humanity did our best to find solutions with the understanding that future generations would have new tools available and would iterate on our solutions. AI may be different. 
...
\begin{itemize}
  \item A framework to think about how we ourselves keep stepping back, contextualizing, as we build AI. Of course, as living creatures, we are often already doing this.
  \item A perspective on what it means for an AI system to keep stepping back. What would it mean for AI to continually release from exclusive attachment to any particular form? How can we create the potential for even \textit{that} conceptualization to be overthrown in the future?
\end{itemize}
