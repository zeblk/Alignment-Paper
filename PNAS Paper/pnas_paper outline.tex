...
\begin{itemize}[leftmargin=2.0em, itemsep=0.3ex]
  ...
  \item No fixed alignment scheme can be aligned. Ongoing discovery is intrinsic to alignment.
\end{itemize}
\end{previewbox}
...
\begin{itemize}
  ...
  \item The systems that succeed and persist are those with barriers on their own patterns of interaction. E.g., precommitment against future impulsivity, plant self-incompatibility, the genome constraining a selfish gene. (In turn, that whole system is a part in another system, and is similarly bounded.) These barrier systems have been fine-tuned by what actually works across long evolutionary history.
  \item Effective barriers are semi-permeable, permitting conditionally stable interactions without letting them become rigid or dominating. A well-functioning barrier does not shut a pattern down, but contextualizes it within a larger system.
  \item Healthy living systems are generative. Barriers cause selection to prefer modular, non-overfit solutions. These become the engine of discovery.
\end{itemize}
...
\begin{itemize}
  \item Humans naturally improve on fixated solutions as we build technology. When a particular technology is overfit (let's say, brakes that only work in dry weather), it eventually gets exposed to situations where it doesn't work, humans see the problem, and the technology is improved.
  \item There are many examples of barriers developed by humans to improve performance in machine learning: `islands' in evolutionary computation; ensembles; multi-agent systems; mixtures of experts; gating mechanisms; local connectivity; slot attention; protection of old memories from new updates; disentangling objectives; stop-gradients and slowly updated targets; regularization against overfitting proxy objectives; withholding the test set.
  ...
  \item There's a trend with increasing generality---towards intelligence---where 
