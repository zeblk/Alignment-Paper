...
\label{abstract}
\noindent
There is broad agreement that the goal of AI alignment is to promote futures full of health and flourishing, but our understanding of what that means and how to reach it remains poor. Here we look to the science of life for guidance. Across scales, from genomes to ecosystems, we observe that notions of health and flourishing consistently map to the ways in which those systems limit fixation on any particular mode of interaction. Fixation is limited by barriers that contextualize modes of interaction, leading to robust modularity and open-ended invention of new structure. With these principles in mind, we turn to the alignment problem. We suggest that to be aligned in a meaningful sense, AI and human-AI systems must include evolving barriers at all scales.
...
In this Perspective, we draw on the science of living systems to better understand how to design and relate to AI in ways that lead to futures full of flourishing and health. We point to a principle of life which has been noted across a variety of systems at different scales. The principle is that living systems are healthy and flourishing when they avoid overly fixating on, or reducing to, particular patterns of interaction. Consistently across types of system, fixation is limited by semi-permeable barriers. Barriers translate shorter-sighted optimization pressure into productive effort, contextualizing individual entities as part of larger structures. Barriers yield deepened individuality, modularity, and open-ended discovery. In Section 1, we work through examples of these mechanics at scales from cells to societies.

Section 2 applies the same principles to the AI alignment problem. There is broad agreement that the aim of alignment---in its broadest sense---is futures full of health and flourishing. From the pattern identified in Section 1, it follows that \textit{alignment means avoiding fixation on particular modes of interaction}. This simple lemma has a surprising consequence: any alignment method, if given too much weight, is necessarily \textit{mis}aligned. Instead, the flourishing of life in the future depends on an evolving set of barriers that continue to limit the influence of any particular approach or concept, creating a generative web of partially competing, partially collaborating perspectives at all scales. Ongoing invention is intrinsic to alignment.

\begingroup
\centering
\vspace{2em}
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.14\textwidth}|>{\raggedright\arraybackslash}p{0.25\textwidth}|>{\raggedright\arraybackslash}p{0.25\textwidth}|>{\raggedright\arraybackslash}p{0.25\textwidth}|}
\hline
\textbf{Example} & \textbf{Too much influence} & \textbf{Barrier} & \textbf{Generativity} \\ \hline

Symbiogenesis & Archea and protobacteria never differentiate & Reproductive isolation between archea and protobacteria & Invention of the eukaryotic cell \\ \hline
...
Brains & Hypersynchrony & Homeostatic plasticity, recurrent inhibition, etc & Behavioral flexibility \\ \hline
...
\end{tabular}
\captionof{table}{Too much influence.}
\endgroup
...
Yet the evolutionary value of genetic exchange depends on both the diversity available and how it is recombined. Mendelian inheritance preserves discrete variants rather than blending them away, but gene flow can homogenize allele frequencies between populations and, when sufficiently strong relative to divergent selection, overwhelm local adaptation \citep{lenormand2002gene}. Partial geographic or reproductive isolation can therefore allow populations to accumulate and retain distinct variants; subsequent hybridization and introgression can bring these variants into new combinations that contribute to rapid speciation and adaptive radiation \citep{marques2019combinatorial}. At the genomic scale, the frequency and location of recombination determine which variants tend to travel together, although genes are not indivisible units: recombination can occur within them \citep{paigen2010mammalian,dooner1997recombination}. Shuffling can assemble beneficial combinations and help selection remove deleterious mutations, but it can also disrupt combinations that work well together, so more exchange is not invariably advantageous \citep{otto2009evolutionary}. Models of genetic ``mixability'' suggest that repeated exposure to different genetic backgrounds can favor alleles that perform well across contexts, providing a possible route to modularity rather than a guarantee that every gene becomes interchangeable \citep{livnat2008mixability,livnat2010sex}. These findings suggest that barriers can contribute to evolutionary innovation by preserving differentiated variation while permitting its recombination; whether such arrangements suffice for open-ended evolution remains an open question.
