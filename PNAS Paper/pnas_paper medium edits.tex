...
In this Perspective, we draw on the science of living systems to better understand how to design and relate to AI in ways that lead to futures full of flourishing and health. We point to a principle of life which has been noted across a variety of systems at different scales. The principle is that living systems are healthy and flourishing when they avoid overly fixating on, or reducing to, particular patterns of interaction. For example, pursuit of a single drive, such as food or sex, becomes pathological for an organism when not balanced with competing drives. At the level of ecosystems, monoculture is fragile and unsustainable. Healthy living systems maximize creative potential at the delicate edge of evolving interplay between diverse, semi-independent entities. Such systems admit many kinds of meaningful description but are not perfectly or eternally captured by any of them. 
...
contextualization. What does that mean? 

modularity and autonomy. how to summarize this succinctly?

Barriers prevent overwriting, but they also support deepening (having to struggle with the world from its own point of view; not getting discharged/short-circuited)

Barriers support individual modularity, and deepened agency, because the thing has to become a generalized model of the world.

holding them in a larger, flexible context. At this sensitive edge, individuality is deepened and individual entities participate in larger structures open-endedly. We then make four conjectures. 


By mitigating fixation, boundaries underpin life's potential to continually discover new solutions and new patterns of organization. In Section 1, we work through examples of fixation and semi-permeable boundaries at scales from cells to societies.



We might have a little semantic trouble, that ``fixation" sounds like something static, while discharging/short-circuiting sounds like something dynamic. 


First, that this is a general principle of life: that to be healthy it must avoid getting stuck on particular arrangements, even dynamic ones. (note in section 1: this is obvious for static arrangements; physicists have known for a long time that life can't be a crystal. is it less obvious for dynamic arrangements? or is that also covered by nonequilibrium thermodynamics?)
...
Our aim is to offer a conceptual lens, which is of course imperfect and temporary. We do not argue for or against AI development or for or against specific alignment methods. Instead, we examine principles that generalize across a breadth of living systems, with the hope they might generalize even to futures substantially different from the present.
...
If any given entity were allowed to act unchecked, it collapses things. At least itself, sometimes more around it. (short-circuiting)

[how does the short-circuiting perspective work for recombination? it's more clear for avoiding self-fertilization...]

`collapsing richly structured interactions'

maybe put metastability already here?

The systems that survive in the long run are the ones that place boundaries on aspects of themselves. This means that, paradoxically, the life that exists now is what has learned to  A dieter holds their own drive for food in check; 


While fixation is unhealthy, same stability is necessary. 

While collapsing into fixation is damaging at the scale at which it happens, it is 

Every entity is partial: it does not capture everything in the world. 

Collapse is relative to a given perspective. One creature's collapse is another's dinner. 

[need to make fixation vs short-circuiting clear.]

(i) contextualization vs.\ overwriting; (ii) boundaries $\rightarrow$ generalization $\rightarrow$ unique perspective; (iii) symmetry breaking as the source of new structure.

semi-permeability caveat carries a lot of weight and appears three times (\S1.4 Kuhn/Popper, \S2.4 ``do not discard schemes capriciously'', \S2.5 awareness does not falsify a belief). It is the paper's main defence against being read as relativism
...
Boundaries guard against fixation, breaking up stationary arrangements and 

situations where information is overwritten

short-circuiting means you have some internal complexity, like a potential energy that's in the form of information in yourself. It distinguishes you from the outside world. But that unique information also inevitably has a drive or motive. If that drive is directly allowed to satisfy itself without resistance, then it immediately dissipates. 

I think Steve is saying that although things are connected, it's also really important that they're partitioned. 

Breaking up stationary arrangements gives parts modularity. 

The rich, open-ended structuration. It's more than just not overwriting something. Each part, by being somewhat walled off, experiences a non-fixed relationship to the rest of the world. \textit{You could always describe this as breaking fixed relationships at the interface between the thing and the outside world.} 

Michael Levin's work on ``multiscale competency architecture" treats entities at every scale as agents with goals in their own problem spaces

The life that exists today, now including humanity's storage and processing of information, is what traced a path through time avoiding both excess and insufficient stability \citep{schrodinger1944what, prigogine1984order, maturana2012autopoiesis}. At the sensitive edge of semi-stability, life open-endedly continues to generate new kinds of organization that require fundamentally new descriptions \citep{kauffman1992origins, stanley2017open, canguilhem1966normal, langton1990computation}.

parts from being dominated by other parts or blended into homogeneity, either of which is fixation to a form admitting less nuanced description. But by allowing interactions, boundaries enable the parts to participate in flexible relationships with one another, contextualizing each part within larger structures---structures that often could not have formed without the diverse parts having undergone their own unique development. The resulting larger system accommodates many partial perspectives rather than locking in a single story of what matters.
...
1) boundaries avoid fixed arrangements $\rightarrow$ keep it dynamic. this makes sex vs asex more natural. we'd have to argue how something like a law against theft is mitigating a ``fixed arrangement". boundaries keep life unsettled and evolving. we'd also have to argue how random noise is a ``fixed arrangement".

2) boundaries avoid short-circuiting $\rightarrow$ keep the internal bouyancy of life. 

3) boundaries allow parts to gain their own unique identities.

Each entity is selected for generality and modularity, at every scale.

By being separated (like markov blankets), the entity inside by definition `wants' something. To maintain continuity of existence (a dynamic and partially competitive world), it has to compensate for the actions of the outside world. So it has to `want' its own existence, under some implicit definition. 

For an entity to have a distinct existence with any continuity over time, it has to want something. The entity exists within a dynamic and partially competitive world. To not get immediately erased, it has to be continually compensating for the actions of the outside world. This means it has to "want" its own existence, under some implicit definition. (Ashby, Friston)

If the entity is in a single context, it overfits to that context. But if it is in many contexts, it has to become a generalized model of them all. It becomes a generalized model of the world \textit{from the unique perspective of its own identity}. All the different entities become different generalized models of the world. 

But because of the boundaries, its ability to fight for that has to work across multiple situations. So it becomes a generalized model of the world \textit{from the unique perspective of its own identity}. All the different entities become different generalized models of the world. 

As the complexity and number of contexts fans out, it becomes easier for the entity to build more generalized models for how all the contexts unfold and relate. In this manner, each entity, if adequately preserved, can contribute a unique perspective, acting not as some disjoint specialized depiction of some fragment of the world but more as a generalized predictor of how the world works.

And then this is happening at all scales, with different perspectives partially exposed to each other through their limited interactions. 
...
[Why does internal locus of control create the freedom for spontaneous generation or creativity?]
...
separate subpopulations of agents with limited exchange between them, to mitigate collapse of diversity \citep{holland1975adaptation, jaderberg2019human}

ensembles of models trained separately \citep{lakshminarayanan2017simple}

multi-agent; separate instances of models with different context, pursuing different problems and goals \citep{hammond2025multi}; limited communication between agents (Jiang \& Lu, NeurIPS 2018; Ding, Huang \& Lu NeurIPS 2020)

mixtures of experts, selectively routing information to subnetworks that specialize in different computations, often with a balancing loss as another boundary against collapse \citep{shazeer2017outrageously}
...
dropout to prevent units within a network from becoming overly co-adapted \citep{srivastava2014dropout}

training objectives that encourage disentangled or sparse latent variables \citep{higgins2017betavae, cunningham2024sparse}
...
search for novelty to boost diversity \citep{lehman2011abandoning}
...
Entities at every scale gain individual autonomy as well as richer integration with larger systems.

In order to be aligned in the long run, AI must become more lifelike. Otherwise, as it has more power, that power will fixate particular forms.
...
To be aligned in any meaningful sense, AI must be lifelike. If you fixate on any particular thing, you become misaligned.
...
Aside from inverse methods, other methods that could work around the reward-misspecification problem but still be incomplete:
* Governance mechanisms

`A kind of alignment analogue of Gödel/Tarski/open-system arguments—i.e. that a competent agent operating in an indefinitely expanding world must encounter morally relevant facts or concepts that were not representable in its original alignment ontology.' -- chatgpt

Preliminary formal attempts at showing that any alignment method is incomplete:
...
As we reviewed with problem solving in groups, there is a simple concept in modeling studies that excessive network connectivity is prone to reaching mediocre solutions quickly and losing the diversity that sustains potential for reaching higher quality solutions over the long run \citep{lazer2007network, zollman2010epistemic}.
...
The closest existing articulation of what we are advocating is polycentric governance \citep{ostrom1961organization, ostrom2010beyond}. In studying how communities manage shared resources, Ostrom and colleagues found that neither a single central authority nor an unstructured market reliably sustains a commons; what works is many overlapping centers of decision-making at different scales, each with real autonomy, each bounded by and accountable to the others. Rules are adapted locally, tested against many situations, and no single center can capture the whole. Our proposal is a generalization of this picture beyond human organizations such as cities, counties and nations. The same logic applies to a cell within a tissue, a gene regulatory network, a species within its niche, a phase of an oscillation in the brain, a subset of active parameters in a mixture-of-experts model, or a single AI agent within a population of humans and other agents---and it must extend to kinds of entities that do not yet exist. What is to be preserved is not any particular list of entities but the structure itself: control distributed richly across a heterarchy of forms \citep{mcculloch1945heterarchy}, in which each entity is contextualized by others without any of them being reducible to, or collapsing into, a single controlling perspective. Alignment, in this view, is the continued elaboration of such distributed control as AI adds new entities and new scales to the world, rather than the convergence of control on any one of them---however benevolent it may appear.
