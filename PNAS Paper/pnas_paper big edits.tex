...
The thesis of Section 1 is that the ongoing health and flourishing of living systems depends on boundaries that limit fixed relationships. For example, 

Healthy living systems have a buoyant internal structure that admits many kinds of description and changes over time. The life that exists today, now including humanity's storage and processing of information, is what traced a path through time avoiding both excess and insufficient stability. At the sensitive edge of semi-stability, life open-endedly continues to generate new kinds of organization that require fundamentally new descriptions \citep{kauffman1992origins, stanley2017open, canguilhem1966normal, langton1990computation, schrodinger1944what, prigogine1984order, maturana2012autopoiesis, simon1962architecture, moreno2015biological}. 

% Under our definitions, such a world is healthier and less fixated than a regular crystal or a homogeneous soup of noise.
...
Mechanistically, to hold parts in context, living systems rely on an extraordinary, many-scale network of semi-permeable boundaries. By limiting interactions, boundaries divide living systems into parts, creating the conditions for each part to refine its own identity---like a writer developing a unique voice or a species adapting to a niche. 
...
We look at a few examples from a range of scales, from genes to institutions.
...
Recombination breaks up associations among loci, allowing selection to act more independently on them \citep{felsenstein1974evolutionary}. Alleles are exposed to varying genetic backgrounds, favoring those that perform well across contexts. [say how this fits the `breaking up fixed relationships point']

An interesting speculation is that, by exposing alleles to many genetic contexts, recombination favors modularity---being able to plug into many possible genomes and serve a useful function \citep{livnat2008mixability,livnat2010sex}. If an allele's function were overfit to a particular genetic context, this brittleness would be penalized at the next recombination. 

% Modularity of one form or another is widely held to promote evolvability \citep{wagner1996perspective,watson2016can}, and the analogy to learning can be made precise: the conditions that prevent overfitting in learning systems predict the conditions under which selection finds generalizable structure rather than quick fixes \citep{kouvaris2017evolution}. Similar to meta-learning in cognitive science and machine learning \citep{wang2018prefrontal}, a blind outer optimization process shapes an inner system to act with foresight.
...
Animals have many innate drives: for nutrition, osmotic balance, temperature, reproduction, avoidance of pain. Each evolved as a proxy for fitness, but each is imperfect. Pursuing one to the exclusion of others paradoxically reduces fitness rather than increasing it \citep{kurthnelson2024dynamic, john2023dead, mcfarland1975behavioural}. Maximizing caloric intake produces obesity; single-minded pursuit of sex produces relational, occupational and legal harm. Higher-order goals behave the same way. Focusing only on work goals can cause burnout, and maximizing reported revenue without regard for honesty or law can lead to imprisonment. Fixating on a particular \textit{strategy} for achieving a goal can even defeat the goal it serves. In a classic experiment, hungry chickens were placed near a cup of food rigged to move in the same direction as the chicken at twice the speed, so that food could only be obtained by running away from it. The chickens were defeated by their own prepotent, overly simplistic strategy of running physically toward the food \citep{hershberger1986approach, dayan2006misbehavior}.
...
[an AI would proxy hack and overfit to a stagnant image of what the world is---which isn't really what the world is]
